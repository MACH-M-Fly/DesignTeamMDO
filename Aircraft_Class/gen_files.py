...
def genMass(AC):
    ...
    f_mass = float(AC.mass)
    f_CG = [float(cg) for cg in AC.CG]
    f_I = [float(i) for i in AC.I]
    out('{:.3f} {:.3f} {:.3f} {:.3f} {:.3f} {:.3f} {:.3f} {:.3f} {:.3f} {:.3f} !	Aircraft'.format(f_mass,
                                                                                                     f_CG[0], f_CG[1], f_CG[2],
                                                                                                     f_I[0], f_I[1], f_I[2], f_I[3], f_I[4], f_I[5]))
...
def genGeo(AC):
    ...
    for i in range(0, len(C)):
        ...
        out(str(Xle[i]) + '    ' + str(Yle[i]) + '    0       ' + str(C[i]) + '     ' + str(
            incAng[i]) + '      ' + '5      3')
        out('AFILE')
    ...
    for i in inds:
        out('SECTION')
        out('#Xle  Yle  Zle  |  Chord   Ainc   Nspan   Sspace')
        out(str(Xle_ht[i]) + '    ' + str(Yle_ht[i]) + '    0       ' + str(C_ht[i]) + '     ' + str(
            0.0) + '      ' + ' ')
        out('NACA')
        out('0012')
        out('CLAF')
